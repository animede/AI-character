...
async def send_stream_payload(websocket: WebSocket, payload: dict, *, send_lock: asyncio.Lock | None = None) -> None:
...
async def send_stream_error(
    ...
    *,
    ...
    if fatal is not None:
        payload["fatal"] = fatal
    await send_stream_payload(websocket, payload, send_lock=send_lock)
...
async def maybe_summarize_turn(
    *,
    conversation_id: str,
    assistant_message_id: str,
    response_text: str,
    ...
    conversation_store.set_message_history_summary(
        conversation_id,
...
async def handle_chat_turn(
    ...
    *,
    ...
    # 1 ターンぶんの履歴保存、LLM ストリーム、TTS 送信、要約、完了通知を担当する。
    ...
    try:
        ...
        async for chunk in stream_chat_chunks(llm_messages):
            ...
            for segment in segmenter.push(chunk):
                await tts_queue.put((segment_index, segment))
                segment_index += 1
