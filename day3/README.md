...
`day3/requirements.txt` には `python-multipart` も含まれています。これはキャラクター登録時の画像・動画アップロードを FastAPI 側で受け取るために必要です。
...
- `新規登録` を押すと新規キャラ作成モードに入ります
- 新規登録中は `キャラクタロール` に標準テンプレートが入り、ボタン表示は `登録中止` に切り替わります
- role に `名前:` 行を書くと、その名前をもとに登録名候補を自動生成します
- `素材登録` を押すと素材編集を開閉できます。開いている間はボタン表示が `素材登録中止` に切り替わります
- 素材登録では、左上のキャラクター表示がアップロード領域になります
- 先に `メイン`、`会話`、`待機` の登録先を選んでから、左上のキャラクター表示をクリックするか、画像・動画ファイルをドラッグ＆ドロップします
- `メイン` には静止画または動画を登録できます。`会話` と `待機` は通常は動画用途ですが、画像を入れた場合も保存自体はできます
- 保存前はローカルプレビューで確認できます
- `キャラ設定を保存` を押すと、設定は `data/characters/<登録名>/character.json` に、素材は `data/characters/<登録名>/assets/{main|talking|waiting}.*` に保存されます
- 保存後の素材は `/api/characters/<登録名>/assets/<種別>` 経由で配信されます
- 編集モードでも `素材登録` を開いている間は、同じ手順で素材を差し替えできます
